...
class sgMysql extends sgDBase{
	...
	public function connect($server="", $user="", $password="", $database="", $port="", $charset="") {
	    ...
	    $this->c = new mysqli($this->server, $this->user, $this->password, $this->database);
	...
	public function describeTable($table){
		...
		if(!$this->c->errno){
			while($rs = $this->result->fetch_array()){
				...
				$info->fields[$field]->type = $aux[1];
				$info->fields[$field]->length = (isset($aux[2])?isset($aux[2]):'');
				$info->fields[$field]->decimals = (isset($aux[3])?isset($aux[3]):'');
				$info->fields[$field]->mtype = $this->getMetaType($aux[1]);
				$info->fields[$field]->not_null = ($rs['Null']=='NO')?true:false;
				$info->fields[$field]->key = ($rs['Key']=='PRI')?true:false;
				$info->fields[$field]->default = $rs['Default'];
				$info->fields[$field]->serial = ($rs['Extra']=='auto_increment')?true:false;
	...
	public function infoQuery($q, $evalMeta=false){
		...
		$info->fieldCount = $this->c->field_count;
		...
		foreach ($fetch_field as $v) {
			...
			$info->fields[$name]->alias = $dup[$f];
			
			
			$info->fields[$name]->orgname = $v->orgname;
			$info->fields[$name]->orgtable = $v->orgtable;
			
			
			$info->fields[$name]->title = $f;
			$info->fields[$name]->type = $v->type;
			$info->fields[$name]->mtype = $this->getMetaType($v->type);
			$info->fields[$name]->length = $v->length;
			$info->fields[$name]->decimals = $v->decimals;
			
			if(isset($des[$t]->fields[$v->orgname])){
				$info->fields[$name]->default = $des[$t]->fields[$v->orgname]->default;
				$info->fields[$name]->not_null = $des[$t]->fields[$v->orgname]->not_null;
			}else{
				$info->fields[$name]->default = $v->def;
				$info->fields[$name]->not_null = ($v->flags & 1)? true: false;
			}


			//$info->fields[$t][$f]->default .= "";
			if(is_null($info->fields[$name]->default)){
				$info->fields[$name]->default = "";
			...
			$info->fields[$name]->key = ($v->flags & 2)? true: false;
			$info->fields[$name]->unique = ($v->flags & 4)? true: false;
			$info->fields[$name]->serial = ($v->flags & 512)? true: false;
			$info->fields[$name]->unsigned = ($v->flags & 32)? true: false;

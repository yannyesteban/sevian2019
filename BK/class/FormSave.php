...
//hr($q);
		...
		if($details){
			...
			foreach($details as $name => $field){
				...
				$s = $field->details;
				//$s->masterData = $this->_data;
					//print_r($data);
				$s->save($this->_data[$name], $this->_data);
	...
	public function save(&$a_data, $dataMaster = array()){
		...
		if(is_array($aux)){
			...
			foreach($a_data as $k => $data){
				$result = $this->saveRecord($a_data[$k], $dataMaster);
			}// next
			...
			while(isset($a_data[$pre.$this->field_mode.$suf.$n])){
				$result = $this->saveRecord($a_data, $dataMaster, $n);
				$n++;
			};
			...
			$result = $this->saveRecord($a_data, $dataMaster);

...
class Postgres extends DBase{
	...
	public function infoTable($table){
		...
		if(!pg_last_error()){
			while($rs = pg_fetch_assoc($result)){
				...
				$field->type = $rs['type'];
				if($rs['type'] == "numeric"){
					$fields->length = $rs['numeric_precision'];
				}else{
					$fields->length = $rs['length'];
				}// end if	
				$field->decimals = $rs['numeric_scale'];
				$field->mtype = $this->getMetaType($rs['type']);
				$field->not_null = ($rs["attnotnull"]=="t")? true: false;
	...
	public function infoResult($result){
		...
		for($i=0; $i < $info->fieldCount; $i++){
			...
			if(!isset($info->tables[$t])){
				...
				$infoTables[$t] = $this->describeTable($t);
	...
	public function infoQueryNO($q, $evalMeta=false){
